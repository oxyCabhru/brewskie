<script lang="ts">
	...
	onMount(() => {
		...
		const inputchips_input: HTMLElement | null = document.querySelector(
	...
	function addChip(e: any) {
		user_choice.update((ch) => {
			...
			if (!dp) return ch;
			dp.type == 'cask' ? ch.casks.push(dp.token) : ch.packages.push(dp.token);
			raw_input = '';
			return ch;
		});
	}
	...
	<div class="controls card variant-glass-primary">
		<div class="brewfile">
			...
			{#key $page_options.autocomplete}
				<div data-popup="autocomplete" class="autocomplete card w-full h-32 overflow-y-auto">
					<Autocomplete bind:input={raw_input} options={autocomp_options} on:selection={addChip} />
				</div>
			{/key}
			<div class="flex flex-row">
				<button
					class={`btn flex flex-row gap-2 ${
						input_choices.length == 0 ? 'variant-soft-secondary' : 'variant-soft-primary'
					}`}
					on:click={view_selections}
				>
					<CheckCheck />
					{$locale.preview_selections[$lang]}
				</button>
				<button
					class={`btn flex flex-row gap-2 ${
						input_choices.length == 0 ? 'variant-soft-secondary' : 'variant-filled-primary'
					}`}
					on:click={fetch_brewskie}
				>
					<FileDown />
					{$locale.download_brewfile[$lang]}
				</button>
			</div>
		</div>
		...
		<div class={`variant-glass relative`}>
			<button
				...
				on:click={() => {
					$page_options.options = !$page_options.options;
				}}
			>
			...
			{#if $page_options.options}
				<div class="flex flex-row justify-around">
					...
					<button
						...
						class={`btn flex flex-col ${
							$page_options.autocomplete ? 'variant-ghost-secondary' : ''
						}`}
						on:click={() => {
							$page_options.autocomplete = !$page_options.autocomplete;
							$page_options.autocomplete ? location.reload() : null;
						}}
					>
					...
					<button
						...
						on:click={() => {
							$page_options.pagination = !$page_options.pagination;
						}}
					>
