<script lang="ts">
	...
	function brewfile() {
		...
		let spl = user_input.split(" ");
		let spl_it = spl.values();
		let ch: UserChoices = {
			packages: [],
			casks: [],
		};
		while (ch.casks.length + ch.packages.length != spl.length) {
			Math.round(Math.random()) ?
				ch.casks.push(spl_it.next().value)
				:
				ch.packages.push(spl_it.next().value)
		};
